...
int main(){
    ...
    int peoplenum,bookstorenum;
    cin >> peoplenum >> bookstorenum;

    MCMF mf(peoplenum+bookstorenum+2,peoplenum+bookstorenum,peoplenum+bookstorenum+1);
    for(int i=0;i<peoplenum;i++){
        int a;
        cin >> a;
        mf.add_edge_to_sink(bookstorenum+i,a,0);
    }
    ...
    for(int i=0;i<bookstorenum;i++){
        for(int j=0;j<peoplenum;j++){
            int a;
            cin >> a;
            mf.add_edge(i,j+bookstorenum,book[i][j],a);
        }
    }
