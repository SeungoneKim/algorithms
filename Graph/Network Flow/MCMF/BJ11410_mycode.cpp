...
int main(){
    ...
    MCMF mf(node+2,0,node+1);
    for(int i=1;i<=node;i++){
        for(int j=i+1;j<=node;j++){
            mf.add_edge(i,j,a[i][j],-c[i][j]);
        }
    }
    for(int i=1;i<=node-1;i++){
        mf.add_edge(i,i+1,1000000000,0);
    }
    mf.add_edge_from_source(1,maxpass,0);
    mf.add_edge_to_sink(node,maxpass,0);
