...
int satisfaction_matrix[4][5]={{0,0,0,0,0},{0,4,3,2,1},{0,8,7,6,5},{0,12,11,10,9}};
...
int main(){
    ...
    while(tc--){
        ...
        MCMF mf(2*node+4,2*node+2,2*node+3);
        // s->s' t'->t
        mf.add_edge_from_source(2*node,2,0);
        mf.add_edge_to_sink(2*node+1,2,0);

        // s'-> G
        for(int i=0;i<node;i++){
            mf.add_edge(2*node,i,1,0);
        }
        // G : node->node
        for(int i=0;i<node;i++){
            mf.add_edge(i,node+i,1,-1);
        }
        // G : edge
        for(int i=0;i<edge;i++){
            ...
            a--;
            b--;
            mf.add_edge(node+a,b,1,0);
        }
        // G->t'
        for(int i=0;i<node;i++){
            mf.add_edge(node+i,2*node+1,1,0);
        }
