...
int main(){
    ...
    //cout << playersnum << '\n';
    MCMF mf(playersnum+4,playersnum+2,playersnum+3);
    for(int i=0;i<playersnum;i++){
        mf.add_edge_from_source(i,1,0);
    }
    for(int i=0;i<playersnum;i++){
        mf.add_edge(i,playersnum,1,-players[i].first);
        mf.add_edge(i,playersnum+1,1,-players[i].second);
    }
    mf.add_edge_to_sink(playersnum,15,0);
    mf.add_edge_to_sink(playersnum+1,15,0);
