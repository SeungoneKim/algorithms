...
int main(){
    ...
    int teamnum=0,a=0,b=0;
    
    while(cin >> teamnum >> a >> b){
        if(teamnum==0 && a==0 && b==0) break;

        MCMF mf(teamnum+4,teamnum+2,teamnum+3);

        mf.add_edge_from_source(teamnum,a,0);
        mf.add_edge_from_source(teamnum+1,b,0);

        for(int i=0;i<teamnum;i++){
            int balloonnum,dista,distb;
            cin >> balloonnum >> dista >> distb;

            mf.add_edge(teamnum,i,balloonnum,dista);
            mf.add_edge(teamnum+1,i,balloonnum,distb);
            mf.add_edge_to_sink(i,balloonnum,0);
        }
